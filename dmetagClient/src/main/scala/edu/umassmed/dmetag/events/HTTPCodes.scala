...
/**
 * HTTP completion codes taken from https://developer.mozilla.org/en-US/docs/Web/HTTP/Status
 */
object HTTPCodes {
  // Starting range of messages
  private val httpInfo = 100
  private val httpSuccess = 200
  private val httpRedirect = 300
  private val httpClientErr = 400
  private val httpServerErr = 500
  private val httpUnknown = 600

  /**
   * Is the HTTP code informational?
   * @param code HTTP code
   * @return true if code informational
   */
  def isInfo(code: Int): Boolean = code >= httpInfo && code <= httpInfo + 99

  /**
   * Is the HTTP code successful?
   * @param code HTTP code
   * @return true if code successful
   */
  def isSuccess(code: Int): Boolean = code >= httpSuccess && code <= httpSuccess + 99

  /**
   * Is the HTTP code a redirect?
   * @param code HTTP code
   * @return true if code a redirect
   */
  def isRedirect(code: Int): Boolean = code >= httpRedirect && code <= httpRedirect + 99

  /**
   * Is the HTTP code a client error?
   * @param code HTTP code
   * @return true if code a client error
   */
  def isClientErr(code: Int): Boolean = code >= httpClientErr && code <= httpClientErr + 99

  /**
   * Is the HTTP code a server error?
   * @param code HTTP code
   * @return true if code a server error
   */
  def isServerErr(code: Int): Boolean = code >= httpServerErr && code <= httpServerErr + 99

  /**
   * Is the HTTP code an unknown error?
   * @param code HTTP code
   * @return true if code an unknown error
   */
  def isUnknown(code: Int): Boolean = code >= httpUnknown

  /**
   * Is the HTTP code ok (information or success)?
   * @param code HTTP code
   * @return true if code ok
   */
  def isOK(code: Int): Boolean = isInfo(code) || isSuccess(code)

  /**
   * Get descriptions (brief and long) for HTTP code
   * @param code HTTP code
   * @return (brief description, long description)
   */
  def getCodeText(code: Int): (String, String) = httpCodes.getOrElse(code, ("Unknown", "Unknown HTTP code"))

  // Map of HTTP code to tuple of (brief description, long explanation)
  private val httpCodes = Map(
